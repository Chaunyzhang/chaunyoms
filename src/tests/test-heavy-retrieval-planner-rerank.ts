...
async function main(): Promise<void> {
  ...
  assert(reranked.audit.used, "deterministic fallback rerank should run when provider is disabled");
  assert(reranked.audit.provider === "deterministic_fallback", "disabled provider should still use deterministic fallback");
  assert(reranked.audit.providerUnavailableReason?.includes("deterministic_fallback"), "audit should record provider unavailability");
  assert(reranked.candidates[0].id === "memory-high", "source-verified active memory should rank first deterministically");

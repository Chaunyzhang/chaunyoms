  ...
  const plan: LlmPlannerPlan = {
    ...
    retrieval: {
      ...
      dagExpansion: {
        ...
        reason: "fixture deterministic expansion",
...
function main(): void {
  ...
  assert(
    ...
    codes(mutated((plan) => {
      ...
      .includes("high_without_source_trace"),

    "high retrieval must require source trace",
  ...
  assert(
    ...
    codes(mutated((plan) => {
      ...
      plan.retrieval.strength = "xhigh";

      plan.retrieval.sourceTraceRequired = true;

      plan.retrieval.candidateLayers = ["base_summaries"];

    })).includes("xhigh_without_raw_sources"),

    "xhigh retrieval must include raw_sources",
  ...
  assert(
    ...
    })).includes("xhigh_summary_only_final_fact"),

    "xhigh retrieval must reject summary-only final fact allowance",
  ...
  const delegatedRepair = validator.validate(mutated((plan) => {

    plan.retrieval.dagExpansion = {
      ...
      reason: "fixture invalid delegated expansion",

    };

  }));

  assert(delegatedRepair.accepted, "delegated DAG expansion without provider should repair instead of rejecting");

  assert(

    delegatedRepair.violations.some((violation) => violation.code === "delegated_dag_expansion_without_provider_repair"),

    "delegated DAG expansion repair warning should be emitted",

  );

  assert(

    delegatedRepair.executablePlan?.retrieval.dagExpansion.mode === "deterministic",

    "delegated DAG expansion without provider must fall back to deterministic execution",
